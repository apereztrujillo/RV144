...
```
  ...
  We would expect to see both graphs of equal size (50% on each dide) if the two are independent. They do not appear to be equal ..shedding doubt on the idea of independence.  
...
We now test the independce of aides vaccine and placebo groups.  

$H_0:$ The placebo and aides vaccines treatment is independent of contracting the disease Aids.
  
  $H_a:$ Having the vaccine is related to contracting the disease. 

The resulting interpretation is the following, if the null hypothesis is indeed true the result we just obtained would happen about `r myrv144$p.value`perecent of the time.
...
$H_0:$ The odds ratio between having aides and getting the vaccine contains the number 1. 

  
  $H_a:$ The odds ration between having aides and getting the vaccine contains does not contain the number 1. 
  
```{r}
myfish <-fisher.test(results)
myfish 
```
The pvalue from the fisher test is `r myfish$p.value` and the confidence interval is `r myfish$p.int`  

## Conclusions

In conlusion like displayed on the bar chart, people who did not get the vaccine or the placebo all got aids. In the group of people that got the vaccine only 40% got aids. The people who got the placebo 60% got aids. That demonstrates that the vaccine does help and makes a positve difference. 